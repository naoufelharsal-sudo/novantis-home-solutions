...
<main id="main">
	...
	<nav class="service-strip" aria-label="Onze diensten">
		<div class="container service-strip-grid">
			<?php foreach ( novantis_services() as $index => $dienst ) : ?>
				<a href="#diensten"><span class="service-icon" aria-hidden="true"><?php echo esc_html( novantis_service_icon( $index ) ); ?></span><span><?php echo esc_html( $dienst['titel'] ); ?></span></a>
			<?php endforeach; ?>
		</div>
	</nav>
			...
			<div class="config-grid" data-configurator>
				<div class="config-panel">
					...
					<div class="package-grid">
						<?php foreach ( array( array( 'solar', '☀', 'Zonnepanelen (± 5 kWp)', 6500, true ), array( 'battery', '▣', 'Thuisbatterij (10 kWh)', 6900, true ), array( 'heatpump', '♨', 'Warmtepomp (lucht-water)', 12500, true ), array( 'charger', 'ϟ', 'Laadpaal', 1800, false ) ) as $item ) : ?>
							<label class="package"><input type="checkbox" value="<?php echo esc_attr( $item[0] ); ?>" data-price="<?php echo esc_attr( $item[3] ); ?>" <?php checked( true, $item[4] ); ?>><span class="package-card"><b class="package-icon"><?php echo esc_html( $item[1] ); ?></b><span><strong><?php echo esc_html( $item[2] ); ?></strong><small>vanaf € <?php echo esc_html( number_format_i18n( $item[3], 0 ) ); ?></small></span></span></label>
						<?php endforeach; ?>
					</div>
				</div>
		...
		<div class="container">
			...
			<div class="cards">
				<?php foreach ( novantis_services() as $dienst ) : ?>
					<article class="card"><img src="<?php echo esc_url( novantis_img( $dienst['img'] ) ); ?>" alt="<?php echo esc_attr( $dienst['titel'] ); ?>" loading="lazy"><div class="card-body"><h3><?php echo esc_html( $dienst['titel'] ); ?></h3><p><?php echo esc_html( $dienst['beschrijving'] ); ?></p><a href="#offerte">Prijs aanvragen →</a></div></article>
				<?php endforeach; ?>
			</div>
		</div>
	</section>
	...
	<section class="section soft-section"><div class="container"><div class="section-heading"><span class="heading-line"></span><h2>Wat klanten zeggen</h2></div><div class="testimonials"><blockquote>“Zonnepanelen, batterij en warmtepomp in één traject. Alles netjes op tijd en de communicatie was top.”<cite>Familie Vermeiren — Antwerpen</cite></blockquote><blockquote>“Volledige dak- en gevelrenovatie. Correcte prijs, duidelijke planning en een prachtig resultaat.”<cite>Kris D. — Mechelen</cite></blockquote><blockquote>“Onze badkamer en elektriciteit volledig vernieuwd. Eén aanspreekpunt maakte het echt zorgeloos.”<cite>Sofie &amp; Tom — Lier</cite></blockquote></div></div></section>
	...
	<section id="offerte" class="section dark-section"><div class="container quote-grid"><div><span class="heading-line"></span><h2>Vandaag investeren, <em>morgen besparen</em></h2><p>Vul het formulier in en we contacteren u binnen 24 uur voor een gratis adviesgesprek en prijsvoorstel.</p><ul class="contact-list"><li><a href="tel:<?php echo esc_attr( preg_replace( '/[^0-9+]/', '', $tel ) ); ?>"><?php echo esc_html( $tel ); ?></a></li><li><a href="mailto:<?php echo esc_attr( $email ); ?>"><?php echo esc_html( $email ); ?></a></li><li>Actief in heel Vlaanderen</li></ul></div><div>
